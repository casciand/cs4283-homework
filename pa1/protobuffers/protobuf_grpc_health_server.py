...
##################################
class ServiceHandler(spb_grpc.HealthServiceServicer):
    ...
    def method(self, request, context):
        ...
        try:
            ...
            print("in method for Service Handler")

            # Now send response
            resp = spb.Response(code=spb.ResponseStatus.OK,
                                    contents="You are Healthy")  # allocate the response object. Note it is empty
...
def driver(port):
    ...
    try:
        ...
        spb_grpc.add_HealthServiceServicer_to_server(handler, server)
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-p", "--port", type=int, default=5577,
                        help="Port where the server part of the peer listens and client side connects to (default: 5577)")
